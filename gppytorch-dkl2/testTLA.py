...
#--Setting Train Data and Test Data
	...
	# Plot training data as black stars
        y1_ax.plot(X_train[0].detach().numpy(), y_train[0].detach().numpy(), 'k*')
        # Predictive mean as blue line
        y1_ax.plot(X_test[0].squeeze().numpy(), mean[0, :].numpy(), 'b')
        # Shade in confidence
        y1_ax.fill_between(X_test[0].squeeze().numpy(), lower[0, :].numpy(), upper[0, :].numpy(), alpha=0.5)
